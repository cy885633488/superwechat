 ...
 * http://10.0.2.2:8080/SuperWeChatServer/Server?request=download_public_groups&m_user_name=&page_id=&page_size=   下载全部的公开群组列表，分页显示的数据
 */
public class DownloadPublicGroupListTask extends BaseActivity {
    private final static String TAG = DownloadPublicGroupListTask.class.getCanonicalName();
    Context mContext;
    String username;
    String path;
    int pageId = 0;
    ...
    public void execute() {
        executeRequest(new GsonRequest<Group[]>(path,Group[].class,reponseDownloadPublicGroupListTaskListener(),errorListener()));
    }

    private Response.Listener<Group[]> reponseDownloadPublicGroupListTaskListener() {
